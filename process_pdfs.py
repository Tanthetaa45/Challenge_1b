...
# Cell 2: Function to parse PDF and extract text (ignoring images)
...
pdf_directory = "sample_dataset\pdfs"  # Create a folder named 'pdfs' and put your PDFs there
output_image_directory = "processed_images"
os.makedirs(output_image_directory, exist_ok=True)

# Load the pre-trained DocLayout-YOLO model
model_path = "model.pt"
if not os.path.exists(model_path):
    ...
    if not pdf_files:
        print(f"No PDF files found in '{pdf_directory}'. Please place your PDFs in this directory.")
    else:
        for pdf_file in pdf_files:
            ...
            for page_num in range(doc.page_count):
                ...
                if pil_image:
                    ...
                    temp_image_path = os.path.join(output_image_directory, f"temp_{pdf_file}_page_{page_num + 1}.jpg")
                    ...
                    try:
                        ...
                        result_image_path = os.path.join(output_image_directory, f"result_{os.path.splitext(pdf_file)[0]}_page_{page_num + 1}.jpg")
            ...
            # Cell 5 (updated): Extract titles, pick largest as document title, cluster remaining to H1/H2/H3, write JSON
import json, re
from collections import defaultdict

# Optional: install scikit-learn if not already available
try:
...
# Use the same DPI you render with
DPI = 200
...
for pdf_file in pdf_files:
    ...
    try:
        for page_num in range(doc.page_count):  # page_num is 0-based
            ...
            temp_image_path = os.path.join(
                output_image_directory, f"temp_{pdf_file}_page_{page_num}.jpg"
            )
            cv2.imwrite(temp_image_path, opencv_image)
    ...
    out_path = os.path.join(OUTPUT_JSON_DIR, f"{os.path.splitext(pdf_file)[0]}.json")
